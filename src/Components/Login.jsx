...
import { Box, FormControl, FormLabel, Button, Input, useToast,FormErrorMessage } from '@chakra-ui/react';
...
export const Login = () => {
    ...
    const handleLogin = async ({email,password}) => {
        try {
            ...
            res = await res.json();
            if (res.token) {
                localStorage.setItem('intechnology', res.token);
                toast({
    ...
    return (
        ...
        email: "",
        password: ""}} onSubmit={(values)=>{handleLogin(values)}} validationSchema={Yup.object({
            ...
            {({handleSubmit,errors,touched})=>(
                ...
                <Field as={Input} variant="filled" type="text" name="email" placeholder='Enter email' isInvalid={errors.email && touched.email}/>
